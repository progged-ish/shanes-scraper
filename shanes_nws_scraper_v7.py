...
def load_nws_stations_csv(csv_path="/home/progged-ish/projects/shanes-scraper/NWS_Stations.csv"):
    """Load NWS stations CSV and return dict mapping station ID to region."""
    import csv
    station_region_map = {}
    try:
        with open(csv_path, 'r') as f:
            ...
            for row in reader:
                # Try both 'Station' and 'Station ID' column names
...
def synthesize_conus_outlook(processed_data, keyword_counts):
    """Generate CONUS summary as expert meteorologist bulletins in plain language."""
    # Collect ALL synoptic discussion text from all offices
    all_synoptic_texts = []
    for state, offices in processed_data.items():
        if state.startswith('_'):
            continue
        for office, data in offices.items():
            if 'synoptic_summary' in data and data['synoptic_summary']:
                # Join the bullet points into paragraph text for better flow
                all_synoptic_texts.extend(data['synoptic_summary'])
    
    if not all_synoptic_texts:
        # Fallback if no synoptic data available
    ...
    # Make a single AI call to generate a unified CONUS expert summary
    print("Generating expert-style CONUS synoptic summary...")
    
    # Concatenate all synoptic text with office context
    full_synoptic_context = "\n\n".join(all_synoptic_texts)
    
    # Prompt for expert-style synoptic bulletin
    expert_prompt = f"""Write a comprehensive CONUS synoptic summary in plain language as an expert meteorologist would write a bulletin. The summary should:

1. Explain the overall synoptic pattern across the entire United States
2. Identify key features (fronts, troughs, ridges, pressure systems, jet streams, etc.)
3. Explain how these features are arranged and their movement
4. Describe the weather implications for different regions
5. Be conversational but professional — like a forecaster briefing colleagues

Use the following synoptic extractions from NWS Forecast Discussions:

{full_synoptic_context}

Format as plain text with clear sections, not markdown. Avoid excessive bullet points; use paragraph form with clear topic sentences."""

    try:
        response = call_fast_model(
            messages=[
                {"role": "system", "content": "You are an expert meteorologist who writes clear, professional synoptic summaries for internal forecast office use."},
        ...
        if "choices" in response and len(response["choices"]) > 0:
            ...
            expert_summary = message.get("content", "").strip()
            
            # Build the full CONUS output
            conus_summary = f"# CONUS Synoptic Summary\n\n"
            conus_summary += f"## Expert Synoptic Summary\n\n{expert_summary}\n\n"
            conus_summary += f"## Regional Overview\n\n"
            
            # Add brief regional breakdown (not full AI summaries to avoid repetition)
            # Just list offices and keyword highlights
            station_region_map = load_nws_stations_csv()
            ...
            for region in sorted(region_offices.keys()):
                ...
                # Get keyword highlights for this region
                region_keywords = [kw for kw, cnt in sorted(keyword_counts.items(), key=lambda x: x[1], reverse=True) 
                                  if any(region.lower() in kw.lower() for region in ['central', 'eastern', 'southern', 'western', 'pacific', 'unknown'] or True)]
                conus_summary += f"### {region} Region\n\n"
                conus_summary += f"Offices: {', '.join(sorted(offices))}\n\n"
                conus_summary += f"Key features: {', '.join(region_keywords[:5]) if region_keywords else 'No significant features'}\n\n"
            
            return conus_summary, {}
            
    except Exception as e:
        print(f"Error generating expert CONUS summary: {e}")
...
# --- V7: Parallel AI Call Helpers ---
def get_fast_model_summary(text: str, office_id: str = None, fast_model_name: str = "gemma-4-e2b-it") -> tuple[str, float]:
    """
    Fast AI summary using GEMMA-4 via LM Studio (parallel worker).
    ...
    try:
        ...
        return (generate_minimal_summary(synoptic_text), 0.0)
    
    return ("[AI call failed]", 0.0)


def get_ai_summary(text: str, office_id: str = None) -> str:
    """
    ...
    Args:
        ...
        office_id: Office ID for timing tracking (optional)
        
    Returns:
        AI-generated summary string (3-4 bullets from Synoptic section)
    """
    # Track timing if office_id provided (placeholder for future enhancement)
    if office_id:
        pass  # Could add timing tracking here
...
def main():
    ...
    # Start wall-clock timing for v7 parallel performance comparison
    ...
    # --- V6: Parallel Processing Loop with ThreadPoolExecutor ---
    office_list = []
    idx = 0
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            # Accumulate timing from per-office results (only count if > 0)
            total_fetch_time += 1.0  # Placeholder - actual fetch time tracked separately
    ...
    # --- V6: Generate CONUS Summary with Regional Breakdown ---
    print("Generating CONUS summary with regional breakdown...")
    ...
    for keyword in sorted(keyword_counts.keys()):
        ...
        for state in sorted(keyword_map[keyword.lower()].keys()):
            ...
            for office in offices:
                ...
                state_abbrev = state_abbreviations.get(state, state).replace(' ', '_')
                # Create link office#state (using keyword for state link)
                link = f'<a href="#{state_abbrev}" class="keyword-link" title="{state}">{office_upper}</a>'
                if link not in keyword_links:
                    keyword_links.append(link)
        if keyword_links:
            keyword_office_links_html += f'<div class="keyword-item"><span class="keyword-label">{keyword.capitalize()}</span><div class="keyword-link-list">{"".join(keyword_links)}</div></div>'
...
"""
    ...
    # --- V5: Assemble Final HUD HTML with Dark Mode ---
    ...
    # Inject CONUS summary at the top of content pane (before state headers)
    conus_summary_html = ""
    if '_conus_summary' in processed_data:
        conus_summary = processed_data['_conus_summary']
        # Convert markdown headers to HTML headings, paragraphs, and line breaks
        import re
        # Convert ### header
        converted_summary = re.sub(r'^### (.+)$', r'<h4 style="color:var(--accent-cyan); margin-top:16px; margin-bottom:8px;">\1</h4>', conus_summary, flags=re.MULTILINE)
        # Convert ## header
        converted_summary = re.sub(r'^## (.+)$', r'<h3 style="color:var(--accent-cyan); margin-top:20px; margin-bottom:12px;">\1</h3>', converted_summary, flags=re.MULTILINE)
        # Convert # header
        converted_summary = re.sub(r'^# (.+)$', r'<h2 style="color:var(--accent-cyan); margin-top:24px; margin-bottom:16px;">\1</h2>', converted_summary, flags=re.MULTILINE)
        ...
        conus_summary_html = f'<div class="conus-summary" style="margin-bottom:32px; padding:20px; background:var(--bg-secondary); border-left:4px solid var(--accent-cyan); border-radius:8px;"><h3 style="color:var(--accent-cyan); margin-bottom:12px;">CONUS SYNOPSIS</h3><div style="font-size:14px; line-height:1.6;">{converted_summary}</div></div>'
    ...
    # Print wall-clock timing for v7 parallel performance comparison
