...
def get_fast_model_summary(text: str, office_id: str = None, fast_model_name: str = "qwen2.5-7b-instruct") -> tuple[str, float]:
    """
    Fast AI summary using qwen2.5-7b-instruct via LM Studio (parallel worker).
    ...
    # Truncate long synoptic texts to fit local model context
    if len(synoptic_text) > 16000:
        synoptic_text = synoptic_text[:16000]
    
    try:
        response_data = call_fast_model(
            messages=[
                {"role": "system", "content": "You are an expert at concise, accurate summarization of NWS synoptic discussions."},
...
<div class="office-card">
    ...
    <div class="office-footer" style="margin-top:10px; padding:8px; background:var(--bg-panel); border-radius:6px; font-size:12px;">
...
"""
    ...
    # Load v8 template
    with open("/home/progged-ish/projects/shanes-scraper/html_templates/modern_dark_template_v8.html", "r") as f:
...
def export_afd_json(processed_data, keyword_counts, execution_time):
    """Export processed AFD data as JSON for the Flask dashboard."""
    export_data = {
