...
def highlight_keywords(text, color_map):
    """Applies colored span tags to keywords in a text string."""
...
def create_folium_map(office_coords):
    """Creates a Folium map with office locations and returns it as an HTML string."""
    map_center = [45, -98.5795]
    m = folium.Map(location=map_center, zoom_start=4, tiles='CartoDB positron')
    for office, coords in office_coords.items():
        popup_html = f"<b>{office}</b><br><a href='#{office}' target='_top'>Go to Discussion</a>"
        folium.CircleMarker(location=coords, radius=5, popup=folium.Popup(popup_html, max_width=200),
                            color='#007bff', fill_color='#007bff', fill=True, fill_opacity=0.7).add_to(m)
    # Add CSS to ensure the map fills the iframe
...
def main():
    ...
    script_version = "2.9.2 (Windows Port)"
    ...
    for state, offices in all_offices_synoptic.items():
        for office in offices:
            ...
            summary, counts = summarize_features(discussion, all_keywords)
            processed_data[state][office] = {'discussion': discussion, 'url': url, 'synoptic_summary': summary}
    ...
    offices_to_plot = {o: c for o, c in nws_office_coords.items() if any(o in ol for ol in all_offices_synoptic.values())}
    map_html = create_folium_map(offices_to_plot)
    ...
    for state in sorted(processed_data.keys()):
        ...
        for office in sorted(processed_data[state].keys()):
            ...
            content_pane_html += f"<hr><h3 id='{office_anchor_id}'>NWS Office: {office.upper()}</h3><h4>Keyword Features:</h4>"
            if data['synoptic_summary']:
                content_pane_html += "<ul>" + "".join([f"<li>{highlight_keywords(item, keyword_colors)}</li>" for item in data['synoptic_summary']]) + "</ul>"
            else:
...
def update_job_status(job_name):
    status_file = r"G:\My Drive\rgroc_status.json"
    data = {}
